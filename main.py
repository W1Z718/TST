...
def main(config):
    ...
    if config['task'] == 'classification':
        ...
        if min_samples < 2:
            logger.warning(f"某些类别样本数量不足2个（最少{min_samples}个）。切换到普通随机抽样而非分层抽样。")
    ...
    if config['val_pattern']:  # used if val data come from different files / file patterns
        val_data = data_data_class(config['data_dir'], pattern=config['val_pattern'], n_proc=-1, config=config)
    ...
    # 获取真实特征维度
    sample_features = my_data.feature_df.iloc[0].values
    if len(sample_features.shape) == 1:
        seq_length = config['max_seq_len'] or 100
        real_feat_dim = ((len(sample_features) + seq_length - 1) // seq_length)  # 向上取整
    else:
        real_feat_dim = sample_features.shape[1]

    # 原始特征维度
    orig_feat_dim = feat_dim
    # 使用实际特征维度替换原来的值
    feat_dim = real_feat_dim
    logger.info(f"调整特征维度: {orig_feat_dim} -> {feat_dim}")
