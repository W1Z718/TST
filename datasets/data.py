...
class CharacterData(BaseData):
    """
    Class for loading and processing CharacterTrajectories dataset
    
    Features:
    - Classification task (20 character classes)
    - 3D time series (x,y coordinates and pen tip pressure)
    - Has original data (_TRAIN.ts/_TEST.ts) and equal length data (_eq_TRAIN.ts/_eq_TEST.ts)
    """
    ...
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        ...
        # Set sample IDs and labels
        self.all_IDs = list(range(len(self.labels_df)))
        ...
        # Set features
        self.feature_names = ['x', 'y', 'pressure']
        self.feature_df = self.all_df[self.feature_names]
        
        # Set class names
        self.class_names = [str(i) for i in range(20)]
        
        # Calculate max sequence length
        self.max_seq_len = self.calculate_max_seq_len()
        
    def calculate_max_seq_len(self):
        """Calculate the length of the longest sequence in the dataset"""
        return max(len(group) for _, group in self.all_df.groupby('ID'))

    def load_all(self, root_dir, pattern=None):
        """Load train or test data"""
        if pattern.upper() == 'TRAIN':
            data_path = os.path.join(root_dir, 'CharacterTrajectories_TRAIN.ts')
        elif pattern.upper() == 'TEST':
            data_path = os.path.join(root_dir, 'CharacterTrajectories_TEST.ts')
        else:
            raise ValueError(f"Unknown pattern: {pattern}")
        
        # Load data
        df = load_from_tsfile_to_dataframe(data_path)
        data = df.iloc[:, :-1]  # 所有列除了最后一列
        ...
        for _, row in data.iterrows():
            curr_len = len(row['dim_0'].values)
            max_len = max(max_len, curr_len)
        
        # 然后填充所有序列到相同长度
        for i, row in data.iterrows():
            # Convert series to numpy arrays with float32 type
            x = np.array(row['dim_0'].values, dtype=np.float32)
            y = np.array(row['dim_1'].values, dtype=np.float32)
            pressure = np.array(row['dim_2'].values, dtype=np.float32)
            
            # 填充到最大长度
            curr_len = len(x)
            if curr_len < max_len:
                x = np.pad(x, (0, max_len - curr_len), mode='constant', constant_values=0)
                y = np.pad(y, (0, max_len - curr_len), mode='constant', constant_values=0)
                pressure = np.pad(pressure, (0, max_len - curr_len), mode='constant', constant_values=0)
            ...
            df.index = pd.MultiIndex.from_product([[i], range(max_len)],
                                                names=['ID', 'timestamp'])
            series_dfs.append(df)
        
        # Merge all sample data
        all_df = pd.concat(series_dfs)
        
        # Process labels
        label_values = []
        for label in labels:
            # 获取标签值并转换为整数（从0开始）
            label_value = int(float(label.iloc[0])) - 1  # 使用 iloc[0] 避免警告
            label_values.append(label_value)
        
        # 转换为整数标签数组
        label_values = np.array(label_values, dtype=np.int8)  # 使用 int8 类型
        
        # Create labels DataFrame
        labels_df = pd.DataFrame({'label': label_values}, index=range(len(label_values)))
...
class DuckDuckGeeseData(BaseData):
    """
    Class for loading and processing DuckDuckGeese dataset
    
    Features:
    - Classification task (5 bird classes)
    - Audio spectogram data
    - Fixed length time series (all samples padded to same length)
    """
    ...
    def __init__(self, root_dir, file_list=None, pattern=None, n_proc=1, limit_size=None, config=None):
        ...
        # 初始化最大序列长度
        self.max_seq_len = 270  # 根据数据集实际情况设置
        ...
        # Set features
        self.feature_names = ['spectogram']  # 音频谱图特征
        self.feature_df = self.all_df[self.feature_names]
    ...
    def load_all(self, root_dir, pattern=None):
        """Load train or test data"""
        if pattern.upper() == 'TRAIN':
            data_path = os.path.join(root_dir, 'DuckDuckGeese_TRAIN.ts')
        elif pattern.upper() == 'TEST':
            data_path = os.path.join(root_dir, 'DuckDuckGeese_TEST.ts')
        else:
            raise ValueError(f"Unknown pattern: {pattern}")
        
        # 定义标签映射字典
        label_map = {
            'black-bellied_whistling_duck': 0,
            'canadian_goose': 1,
            'greylag_goose': 2,
            'pink-footed_goose': 3,  # 注意这里的连字符
            'white-faced_whistling_duck': 4
        }
        
        # Load data using sktime's ts file loader
        ...
        print(f"Max sequence length: {max_len}")  # 添加调试信息
        
        # 然后填充所有序列到相同长度
        for i, row in data.iterrows():
            # Convert series to numpy arrays with float32 type
            spectogram = np.array(row['dim_0'].values, dtype=np.float32)
            
            # 填充到最大长度
            curr_len = len(spectogram)
            if curr_len < max_len:
                spectogram = np.pad(spectogram, (0, max_len - curr_len), 
        ...
        # Process labels - 使用标签映射
        label_values = []
        print("\nProcessing labels:")  # 添加调试信息
        for i, label in enumerate(labels):
            print(f"Sample {i}, Original label: {label}")  # 添加调试信息
            
            # 如果标签是空的 Series，尝试从文件名或其他地方获取标签
            if isinstance(label, pd.Series) and len(label) == 0:
                # 假设每个类别有相同数量的样本，这里是20个
                class_idx = i // 20
                label_value = class_idx
            else:
                # 将字符串标签转换为数值
                if isinstance(label, str):
                    label_value = label_map.get(label.lower(), -1)
                    if label_value == -1:
                        print(f"Warning: Unknown label '{label}' encountered.")
                        continue
                else:
                    label_value = int(label) - 1 if isinstance(label, (int, float)) else -1
                    if label_value < 0:
                        continue
            
            print(f"Converted to label_value: {label_value}")  # 添加调试信息
            label_values.append(label_value)
        ...
        print(f"\nTotal valid labels found: {len(label_values)}")  # 添加调试信息
        
        # 转换为整数标签数组
        label_values = np.array(label_values, dtype=np.int8)
...
class Data3004(TSRegressionArchive):
    """3004代码错误分类数据"""
    
    def __init__(self, root_dir, file_list=None, pattern=None, **kwargs):
        super().__init__(root_dir, file_list=['code_errors_ast.ts'], **kwargs)

class Code3004ExecutionTraceData(TSRegressionArchive):
    """3004执行轨迹数据"""
    ...
    def __init__(self, root_dir, file_list=None, pattern=None, **kwargs):
        ...
        if not os.path.isdir(root_dir):
            # 如果路径不存在，尝试查找上级目录
            parent_root_dir = os.path.join("..", root_dir)
            if os.path.isdir(parent_root_dir):
...
data_factory = {
    ...
    'duckduckgeese': DuckDuckGeeseData,
    '3004': Data3004,  # 添加AST特征数据类
    '3004_execution_trace': Code3004ExecutionTraceData  # 添加执行轨迹数据类
}
