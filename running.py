...
def setup(args):
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)  # 鑷?鍔ㄥ垱寤虹洰褰?
...
def evaluate(evaluator, epoch_num=None, keep_all=True):
    ...
    for i, batch in enumerate(evaluator.dataloader):
        ...
        per_batch['metrics'].append(loss.detach().cpu().numpy())  # 鐩存帴瀛樺偍loss鏁扮粍
    ...
    if keep_all:
        ...
        per_batch['metrics'] = np.concatenate(per_batch['metrics'], axis=0)  # 鐩存帴杩炴帴loss鏁扮粍
...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    if condition:
        ...
        # 澶勭悊 per_batch 瀛楀吀锛岀'淇濇墍鏈夋暟缁勯兘鏄?瑙勫垯鐨勫舰鐘?
        save_dict = {}
...
class SupervisedRunner(BaseRunner):
    ...
    def __init__(self, model, dataloader, device, loss_module, optimizer=None, l2_reg=None, print_interval=10, console=True):
        ...
        # 娣诲姞宓屽叆淇濆瓨鐩稿叧鐨勫弬鏁?
        self.save_embedding_interval = 1  # 姣忎釜epoch閮戒繚瀛?
        self.config = None
    ...
    def train_epoch(self, epoch):
        ...
        for i, batch in enumerate(self.dataloader):
            # 鐜板湪batch鍙?鍖呭惈3涓?鍏冪礌锛歠eatures, targets, padding_masks
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        # 计算每个类别的precision, recall和F1，处理警告
