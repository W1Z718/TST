...
def model_factory(config, data):
    ...
    # 检查并获取实际特征维度
    try:
        # 使用数据集的第一个样本来获取实际特征维度
        sample_features = data.feature_df.iloc[0].values
        if len(sample_features.shape) == 1:
            # 如果特征是一维的，需要重塑
            seq_length = 100
            feat_dim = ((len(sample_features) + seq_length - 1) // seq_length)  # 向上取整
        else:
            # 如果特征已经是二维的
            feat_dim = sample_features.shape[1]
    except:
        # 如果无法获取样本特征，使用传入的特征维度
        feat_dim = data.feature_df.shape[1]
        print(f"警告: 使用默认特征维度 {feat_dim}，可能不正确")
    
    print(f"实际特征维度: {feat_dim}")
...
class TSTransformerEncoderClassiregressor(nn.Module):
    """
    Transformer encoder for classification/regression, with ability to extract embeddings
    """
    ...
    def forward(self, X, padding_masks, return_embedding=False):
        """
        Args:
            X: (batch_size, seq_length, feat_dim) 输入特征
            padding_masks: (batch_size, seq_length) 1表示有效, 0表示填充
            return_embedding: 是否返回嵌入表示
        """
        # 检查输入维度并修复
        if len(X.shape) == 2:  # 如果输入是二维的 [batch_size, seq_length]
            batch_size, seq_len = X.shape
            # 添加一个特征维度
            X = X.unsqueeze(-1)
            print(f"警告: 输入维度被调整为三维 {X.shape}")
        elif len(X.shape) != 3:
            raise ValueError(f"输入维度不支持: {X.shape}, 期望 [batch_size, seq_length, features]")
        ...
        # 应用掩码
        masked_output = output * padding_masks.unsqueeze(-1)  # zero-out padding embeddings
        
        # 加权池化而不是简单平均
        attn_weights = torch.softmax(self.output_layer[0](masked_output), dim=1)
        embedding = torch.sum(masked_output * attn_weights, dim=1)
        
        # 获取分类输出
        class_output = self.output_layer(embedding)  # (batch_size, num_classes)

        if return_embedding:
            return class_output, embedding
        return class_output
    ...
    def get_embeddings(self, dataloader, device):
        ...
        with torch.no_grad():
            for batch in dataloader:
                features, y, padding_masks = batch
                features = features.to(device)
                padding_masks = padding_masks.to(device)
                
                _, batch_embedding = self.forward(features, padding_masks, return_embedding=True)
                
                embeddings.append(batch_embedding.cpu())
                labels.append(y)
                
                # 注意：这里移除了ids，因为当前batch中不再返回
                # 如果需要保留ID信息，需要修改dataset和collate函数
        ...
        return {
            ...
            'labels': labels,
            'ids': list(range(len(embeddings)))  # 使用索引替代
        }
